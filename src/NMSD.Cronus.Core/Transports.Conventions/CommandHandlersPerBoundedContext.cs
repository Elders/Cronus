...
namespace NMSD.Cronus.Core.Transports.Conventions
{
    public class CommandHandlersPerBoundedContext : ICommandHandlerEndpointConvention
    {
        ...
        public IEnumerable<EndpointDefinition> GetEndpointDefinitions(params Type[] handlerTypes)
        {
            ...
            string endpointName = String.Format("{0}.CommandHandlers", boundedContext.BoundedContextNamespace);
            ...
            foreach (Type handlerType in handlerTypes)
            {
                ...
                var eventIds = messageTypes.Select(x => new Guid((x.GetCustomAttribute(typeof(DataContractAttribute), false) as DataContractAttribute).Name));
